...
# 2. Чистка и преобразование переменных - гестация, длина тела, масса тела, мозг, сердце, лёгкие, печень, мацерация, ВПР - к нужному типу (ГЛАВНЫЙ ЧАНК)
```{r, results='hide'}
# Загрузка данных и преобразование в tibble:

df <- read_csv("aged_data_full2.csv")
df <- as_tibble(df) 
...
df <- df %>%
  mutate(across(c(`мацерация`,`ВПР`), function(x) as.factor(x)))%>%
...
# Фильтрация и создание нового датасета референсных значений df1 
```{r, results='hide'}
...
# Замена пропусков методом knn, k = 4
```{r, warning = FALSE }


df1_init <- as.data.frame(df1)

# k - квадратный корень числа переменных 4
...
df1_ksqrt <- predict(preProcValues, df1_init, na.action = na.pass)
...
for(i in procNames$col){
 df1_ksqrt[i] <- df1_ksqrt[i]*preProcValues$std[i]+preProcValues$mean[i] 
}


df1_ksqrt$df_type <- "DF k=4"
df1_init$df_type <- "Исходный DF"


df1_all <- rbind(df1_init, df1_ksqrt)


all_num <- df1 %>% 
  select(where(is.numeric))

vec_var <- names(all_num)


for (i in 1:length(vec_var)){
  
ggplot(df1_all, aes(sample = df1_all[,vec_var[i]], color = as.factor(df_type))) +
  stat_qq() +
  stat_qq_line() +
  labs(color = "Тип импутации") +
  ggtitle(paste("Q-Q plot для переменной", vec_var[i])) + xlab("Теоретические") + ylab("Выборочные")
  
ggsave(paste(vec_var[i],"na_imp_k4",".png"), 
  width = 8.5, 
  height = 9, 
  dpi = 100)
}
...
# k - логарифм числа переменных числа переменных 2.8~3
```{r, warning = FALSE }
...
df1_kln <- predict(preProcValues, df1_init, na.action = na.pass)
...
procNames <- data.frame(col = names(preProcValues$mean), mean = preProcValues$mean, sd = preProcValues$std)
for(i in procNames$col){
 df1_kln[i] <- df1_kln[i]*preProcValues$std[i]+preProcValues$mean[i] 
}


df1_kln$df_type <- "DF k=3"

df1_all <- rbind(df1_init, df1_kln)



for (i in 1:length(vec_var)){
  
qqplot <- ggplot(df1_all, aes(sample = df1_all[,vec_var[i]], color = as.factor(df_type))) +
  stat_qq() +
  stat_qq_line() +
  labs(color = "Тип импутации") +
  ggtitle(paste("Q-Q plot для переменной", vec_var[i])) + xlab("Теоретические") + ylab("Выборочные")
  ...
  #Сохранение графиков
  ggsave(plot = qqplot, paste(vec_var[i],"na_imp_k3",".png"), 
  width = 8.5, 
  height = 9, 
  dpi = 100)
...
df1_k5 <- predict(preProcValues, df1_init, na.action = na.pass)
...
procNames <- data.frame(col = names(preProcValues$mean), mean = preProcValues$mean, sd = preProcValues$std)
for(i in procNames$col){
 df1_k5[i] <- df1_k5[i]*preProcValues$std[i]+preProcValues$mean[i] 
}


df1_k5$df_type <- "DF k=5"

df1_all <- rbind(df1_init, df1_k5)
...
#Сохранение графиков
ggsave(plot = qqplot, paste(vec_var[i],"na_imp_k5",".png"), 
       width = 8.5, 
       height = 9, 
       dpi = 100)
...
#Отдельно дл поджелудочной железы

# k - равно 10, 15, 20, 25, 30
preProcValues <- preProcess(df1_init %>% 
                          dplyr::select(where(is.numeric)),
                            method = c("knnImpute"),
                            k = 30,
...
df1_k10 <- predict(preProcValues, df1_init, na.action = na.pass)
...
procNames <- data.frame(col = names(preProcValues$mean), mean = preProcValues$mean, sd = preProcValues$std)
for(i in procNames$col){
 df1_k10[i] <- df1_k10[i]*preProcValues$std[i]+preProcValues$mean[i] 
}


df1_k10$df_type <- "DF k=30"

df1_all <- rbind(df1_init, df1_k10)
...
ggsave(plot = qqplot, paste(vec_var[i],"na_imp_k30",".png"), 
       width = 8.5, 
       height = 9, 
       dpi = 100)
...
#Добавить группу всего!!!!!!!

# Описательная статистика с разными методами импутации данных методом knn
```{r, warning = FALSE }
...
stat_num_res %>% 
  ...
  set_caption("Описательная статистика количественных переменных для исходного набора данных") %>%
  width(width = 1.2)

#Импутация

df1_ksqrt_all <- df1_ksqrt %>%
  mutate(`мёртвый/живой` = "Всего")

df1_ksqrt_all <- rbind(df1_ksqrt_all,df1_ksqrt)

stat_num_res <- df1_ksqrt_all %>%
  select(`мёртвый/живой`, where(is.numeric)) %>%
  group_by(`мёртвый/живой`) %>%
  summarise(across(where(is.numeric), statistics)) %>%
  mutate(across(everything(), as.character)) %>% 
...
stat_num_res %>% 
  ...
  theme_box() %>%
  merge_v(c( "Переменная")) %>%
  set_caption("Описательная статистика количественных переменных для набора данных с импутаией knn (k=4)") %>%
  width(width = 1.2)
...
stat_num_res %>% 
  ...
  theme_box() %>%
  merge_v(c( "Переменная")) %>%
  set_caption("Описательная статистика количественных переменных для набора данных с импутаией knn (k=3)") %>%
  width(width = 1.2)
...
stat_num_res %>% 
  ...
  theme_box() %>%
  merge_v(c( "Переменная")) %>%
  set_caption("Описательная статистика количественных переменных для набора данных с импутаией knn (k=5)") %>%
  width(width = 1.2)
...
```{r, warning = FALSE }
#Сохранение графиков по 4 метода на одном
df1_all <- rbind(df1_init, df1_ksqrt , df1_kln ,df1_k5)


for (i in 1:length(vec_var)){
  
qqplot <- ggplot(df1_all, aes(sample = df1_all[,vec_var[i]], color = as.factor(df_type)), alpha=0.2) +
  stat_qq() +
  stat_qq_line() +
  labs(color = "Тип импутации") +
  ggtitle(paste("Q-Q plot для переменной", vec_var[i])) + xlab("Теоретические") + ylab("Выборочные")
  ...
  #Сохранение графиков
  ggsave(plot = qqplot, paste(vec_var[i],"na_imp_knn",".png"), 
  width = 8.5, 
  height = 9, 
  dpi = 100)
